<x-app-layout>
    @section('header', 'Tambah Tes Seleksi')

    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900">
                    <form action="{{ route('panitia.tests.store') }}" method="POST">
                        @csrf
                        <div class="mb-4">
                        ...
                        <div class="mb-4">
                            ...
                            <select name="type" id="type" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm" required>
                                ...
                                <option value="mengaji" {{ old('type') == 'mengaji' ? 'selected' : '' }}>Tes Mengaji</option>
                                <option value="wawancara" {{ old('type') == 'wawancara' ? 'selected' : '' }}>Tes Wawancara</option>
                            </select>
                            @error('type')
                                <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                            @enderror
