    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="flex justify-between items-center mb-8">
                ...
                <form action="{{ route('dashboard') }}" method="GET" class="flex items-center space-x-2 bg-white p-2 rounded-lg shadow-sm">
                    <label for="year" class="text-sm font-medium text-gray-700">Tahun:</label>
                    <select name="year" id="year" onchange="this.form.submit()" class="text-sm border-gray-300 rounded-md shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
...
<div class="grid grid-cols-1 md:grid-cols-1 gap-6 mb-8">
    <a href="{{ route('panitia.jadwal.index') }}" class="bg-white overflow-hidden shadow-sm sm:rounded-lg p-6 border-b-4 border-indigo-500 hover:bg-indigo-50 transition-colors group">
                        ...
                        @foreach ($recentSiswa as $siswa)
                            <div class="bg-gray-50 p-4 rounded-2xl border border-gray-100 shadow-sm">
                                <div class="flex justify-between items-start mb-4">
                                    ...
                                    <span class="px-2 py-1 text-[10px] uppercase tracking-wider font-bold rounded-lg bg-{{ $siswa->status === 'diterima' ? 'green' : ($siswa->status === 'ditolak' ? 'red' : 'yellow') }}-100 text-{{ $siswa->status === 'diterima' ? 'green' : ($siswa->status === 'ditolak' ? 'red' : 'yellow') }}-800">
                                        {{ $siswa->status }}
                                    </span>
                                </div>

                                <div class="flex items-center justify-between pt-4 border-t border-gray-100">
                                    <span class="text-xs text-gray-500">{{ $siswa->created_at->format('d M Y') }}</span>
