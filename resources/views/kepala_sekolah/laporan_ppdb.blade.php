    ...
    <div class="space-y-6">
        ...
        <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
            <div class="flex flex-wrap items-end justify-between gap-4 mb-4">
                ...
                <a href="{{ route('kepala_sekolah.laporan_ppdb.export', request()->query()) }}" class="inline-flex items-center px-4 py-2.5 bg-green-600 text-white rounded-lg hover:bg-green-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-green-500 transition-colors font-medium shadow-sm">
            ...
            <form method="GET" action="{{ route('kepala_sekolah.laporan_ppdb.index') }}" class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5 gap-4">
                <!-- Search -->
                <div>
                    <label for="search" class="block text-sm font-medium text-gray-700 mb-2">Cari</label>
                    <input type="text" name="search" id="search" value="{{ request('search') }}" placeholder="Nama atau NISN..." class="w-full rounded-lg border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
                </div>
                ...
                <div>
                    ...
                    <select name="year" id="year" class="w-full rounded-lg border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
                ...
                <div class="md:col-span-2 lg:col-span-3 xl:col-span-5 flex flex-wrap gap-2">
                    ...
                    <a href="{{ route('kepala_sekolah.laporan_ppdb.index') }}" class="px-6 py-2.5 bg-gray-100 text-gray-700 rounded-lg hover:bg-gray-200 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-gray-500 transition-colors font-medium">
                        <svg class="h-5 w-5 inline mr-1" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                        ...
                        <tr>
                            <td colspan="7" class="px-6 py-12 text-center">
                                <div class="flex flex-col items-center justify-center text-gray-500">
                                    <svg class="h-12 w-12 mb-3 text-gray-400" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                ...
                @forelse($siswas as $siswa)
                    <div class="bg-white p-4 rounded-2xl border border-gray-100 shadow-sm transition-all duration-200">
                        <div class="flex justify-between items-start mb-4">
                            ...
                            @if($siswa->status === 'diterima')
                                <span class="px-2 py-1 text-[10px] uppercase font-bold rounded-lg bg-green-100 text-green-800">Diterima</span>
                            @elseif($siswa->status === 'ditolak')
                                <span class="px-2 py-1 text-[10px] uppercase font-bold rounded-lg bg-red-100 text-red-800">Ditolak</span>
                            @else
                                <span class="px-2 py-1 text-[10px] uppercase font-bold rounded-lg bg-yellow-100 text-yellow-800">Pending</span>
                            @endif
                        </div>
                        ...
                        <div class="space-y-3 pt-3 border-t border-gray-50">
                            <div class="flex items-center justify-between">
                                ...
                                <span class="text-xs font-medium text-gray-700">{{ Str::limit($siswa->asal_sekolah, 25) }}</span>
                            </div>
                            <div class="flex items-center justify-between">
                            ...
                            <div class="flex items-center justify-between">
                                ...
                                <span class="text-xs font-medium text-gray-400">{{ $siswa->created_at->format('d/m/Y H:i') }}</span>
                            </div>
                        </div>
                    </div>
                @empty
