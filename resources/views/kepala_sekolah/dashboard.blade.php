<x-app-layout>
            ...
            <div class="bg-white p-6 rounded-2xl shadow-sm border border-gray-100 h-full flex items-center">
                <form action="{{ route('dashboard') }}" method="GET" class="flex flex-col gap-2">
                    ...
                    <div class="flex items-center gap-2">
                        <select name="year" id="year" class="rounded-xl border-gray-200 text-gray-700 focus:ring-indigo-500 focus:border-indigo-500 min-w-[120px]">
                            @foreach($availableYears as $year)
                                <option value="{{ $year }}" {{ $selectedYear == $year ? 'selected' : '' }}>{{ $year }}</option>
                            @endforeach
                        </select>
                        <button type="submit" class="bg-indigo-600 text-white px-4 py-2 rounded-xl hover:bg-indigo-700 transition-colors shadow-sm">
                            Filter
                        </button>
                    </div>
                </form>
 ...
 <!-- Statistik Pendaftaran per Tahun -->
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            <a href="{{ route('kepala_sekolah.laporan_ppdb.index') }}" class="bg-white rounded-xl shadow-sm border border-gray-100 p-6 hover:shadow-md transition-all hover:border-indigo-200 group">
                <div class="flex items-center">
                    <div class="h-12 w-12 bg-indigo-100 rounded-xl flex items-center justify-center group-hover:bg-indigo-200 transition-colors">
                        <svg class="h-6 w-6 text-indigo-600" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 17v-2m3 2v-4m3 4v-6m2 10H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" />
                        </svg>
                    </div>
                    <div class="ml-4">
                        <h4 class="text-lg font-semibold text-gray-900 group-hover:text-indigo-600 transition-colors">Laporan PPDB</h4>
                        <p class="text-sm text-gray-500">Lihat laporan lengkap pendaftaran</p>
                    </div>
                </div>
            </a>

            <a href="{{ route('users.index') }}" class="bg-white rounded-xl shadow-sm border border-gray-100 p-6 hover:shadow-md transition-all hover:border-indigo-200 group">
                <div class="flex items-center">
                    <div class="h-12 w-12 bg-violet-100 rounded-xl flex items-center justify-center group-hover:bg-violet-200 transition-colors">
                        <svg class="h-6 w-6 text-violet-600" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4.354a4 4 0 110 5.292M15 21H3v-1a6 6 0 0112 0v1zm0 0h6v-1a6 6 0 00-9-5.197M13 7a4 4 0 11-8 0 4 4 0 018 0z" />
                        </svg>
                    </div>
                    <div class="ml-4">
                        <h4 class="text-lg font-semibold text-gray-900 group-hover:text-violet-600 transition-colors">Kelola Pengguna</h4>
